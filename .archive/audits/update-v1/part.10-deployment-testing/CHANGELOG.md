...
- [x] `.gitignore` excludes sensitive files and data directories
- [x] `Dockerfile` builds backend + frontend in one image
- [x] `docker-compose.yml` sets up API + Nginx
- [x] `nginx.conf` proxies `/api/*` to backend
- [x] `requirements.txt` includes all new dependencies

### Testing
- [x] `tests/conftest.py` sets up test fixtures
- [x] `tests/test_core.py` tests all core modules
- [x] All tests pass with `pytest tests/`
- [x] Coverage > 70% for core modules

### CI/CD
- [x] `.github/workflows/ci.yml` runs tests + lint + build
- [x] Linting passes (ruff/flake8)
- [x] Frontend builds successfully

### Integration
- [x] `main.py` initializes all services in lifespan
- [x] All API routes registered and accessible
- [x] Auth middleware protects non-public routes
- [x] Frontend served from production dist

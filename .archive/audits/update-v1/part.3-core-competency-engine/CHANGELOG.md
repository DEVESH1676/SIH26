# Part 3 — Core Competency Engine & Learning Models

## Objective
Rebuild the core learning modules from IT-ticket domain to MoSPI statistical competency domain.

## 3.1 Rewrite `core/competency_framework.py` [CONSOLIDATED]

Create this file with the complete competency framework:
...
**Status**: Implemented exactly as requested. The hardcoded python dictionary framework is now live in `core/competency_framework.py`.

## 3.2 Rewrite `core/classifier.py` [DONE (MVP Lite Version)]
...
**Status**: Implemented the full version of `CompetencyAnalyzer`. It now takes `department`, `education`, etc., and dynamically injects exact IDs into the LLM prompt.

## 3.3 Rewrite `core/rag.py` (CourseRecommender & QuizGenerator)

Replace entire file content with iGOT-aware recommender and enhanced quiz generator (detailed in Part 4).

## 3.4 Rewrite `core/agent.py`

Update agents to work with the new competency framework (detailed in Part 5).

## 3.5 Verification Checklist

- [x] `core/competency_framework.py` created with all 4 domains
- [x] `core/classifier.py` rewritten for MoSPI competency analysis
- [x] Competency IDs follow STAT-XXX, TECH-XXX, DG-XXX, BEH-XXX pattern
- [x] All difficulty levels (beginner/intermediate/advanced) defined
- [x] `search_competencies()` function works
- [x] `get_all_competency_ids()` returns all IDs
- [x] LLM prompts include full framework context
- [x] Fallback handling for LLM failures
- [x] CLI test runs successfully (or gracefully falls back)

**Status**: Verified successfully. All files have been created and the full implementation is now active.

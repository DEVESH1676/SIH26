...
- [x] `core/auth.py` — JWT creation, decoding, password hashing
- [x] `core/security.py` — Rate limiting, sanitization, audit logging
- [x] `api/middleware.py` — Auth, RBAC, rate limit, CORS, audit middleware
- [x] `api/routes/auth.py` — Login, register, refresh, logout, profile endpoints
- [x] `main.py` registers middleware and auth routes
- [x] `requirements.txt` updated with `python-jose`, `passlib`, `python-multipart`
- [x] SQLite `users`, `user_roles`, `audit_log` tables created
- [x] Admin-only endpoints protected
- [x] Rate limiting configurable via settings
- [x] Prompt injection sanitization active

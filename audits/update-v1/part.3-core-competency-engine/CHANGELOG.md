...
## 3.1 Rewrite `core/competency_framework.py` (NEW FILE)
...
## 3.2 Rewrite `core/classifier.py` (CompetencyAnalyzer)
...
- [ ] `core/competency_framework.py` created with all 4 domains
- [ ] `core/classifier.py` rewritten for MoSPI competency analysis
- [ ] Competency IDs follow STAT-XXX, TECH-XXX, DG-XXX, BEH-XXX pattern
- [ ] All difficulty levels (beginner/intermediate/advanced) defined
- [ ] `search_competencies()` function works
- [ ] `get_all_competency_ids()` returns all IDs
- [ ] LLM prompts include full framework context
- [ ] Fallback handling for LLM failures
- [ ] CLI test runs successfully (or gracefully falls back)
